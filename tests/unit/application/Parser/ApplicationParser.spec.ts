  ...
  describe("parseApplication", () => {
    describe("categoryParser", () => {
      it("returns result from the parser", () => {
        ...
        const parser = new CategoryCollectionParserStub().withReturnValue(data, expected).getStub();
    ...
    describe("projectDetails", () => {
      it("projectDetailsParser is used to create the instance", () => {
        ...
        const projectDetailsParserStub = new ProjectDetailsParserStub().withReturnValue(
          expectedProjectDetails
        );
        const sut = new ApplicationParserBuilder().withProjectDetailsParser(
          projectDetailsParserStub.getStub()
        );
        // act
        const app = sut.parseApplication();
        // assert
    ...
    describe("collectionsData", () => {
      describe("set as expected", () => {
        ...
        // act
        testScenarios.forEach(({ description, input, output }) => {
          it(description, () => {
            let categoryParserStub = new CategoryCollectionParserStub();
            for (let i = 0; i < input.length; i++) {
              categoryParserStub = categoryParserStub.withReturnValue(input[i], output[i]);
            }
            const sut = new ApplicationParserBuilder()
...
class ApplicationParserBuilder {
  private categoryCollectionParser: CategoryCollectionParser =
    new CategoryCollectionParserStub().getStub();

  private projectDetailsParser: typeof parseProjectDetails =
    new ProjectDetailsParserStub().getStub();
  ...
  public withCategoryCollectionParser(categoryCollectionParser: CategoryCollectionParser): this {
    this.categoryCollectionParser = categoryCollectionParser;
    return this;
  }

  public withProjectDetailsParser(projectDetailsParser: typeof parseProjectDetails): this {
  ...
  public parseApplication(): ReturnType<typeof parseApplication> {
    return parseApplication(this.collectionsData, {
